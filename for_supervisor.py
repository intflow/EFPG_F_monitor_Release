        ...
        try: # 이 자리에 시간마다 처리하고 싶은 코드를 집어 넣으면 됨.
            ...
            if free < total * ALLOW_CAPACITY_RATE:
                ...
                while (max_day_cnt >= -1):
                    ...
                    # folder 내부 날짜순으로 제거
                    os.system(f"sudo -S find {_path_} -name '*.mp4' -ctime +{max_day_cnt}" + " -exec rm -rf {} \;")
...
if __name__ == "__main__":
    try:
        ...
        # metadata 권한 변경.
        subprocess.run(f" sudo -S chown intflow:intflow -R {configs.METADATA_DIR}", shell=True)
        subprocess.run(f" sudo -S chmod 775 -R {configs.METADATA_DIR}", shell=True)
        now_dt = dt.datetime.now().astimezone(dt.timezone(dt.timedelta(hours=9)))
        print(now_dt.hour)
        ...
        while (True):
            ...
            if not check_deepstream_status():
